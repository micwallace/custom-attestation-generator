...
import {base64ToUint8array, hexStringToUint8, uint8arrayToBase64, uint8tohex} from "@tokenscript/attestation/dist/libs/utils";
	...
	private generateSchema(): any {
		...
		for (let i in this.jsonSchema){
			...
			if (this.jsonSchema[i].items){
				...
				if (this.jsonSchema[i].name === "Validity"){
					console.log(this.jsonSchema[i])
				}
